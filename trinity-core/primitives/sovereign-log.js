...
export function emit(event) {
  ...
  const payload = { type, seq: _seq++, ts: (globalThis.__now ? globalThis.__now() : Date.now()), ...event };
...
export function deriveState(log = _log) {
  ...
  for (const e of log) {
    switch (e.type) {
      ...
      case EVENT_TYPES.FORK_RESOLVED:
        forkResolutions = forkResolutions.concat({
          height:       e.height,
          winningHash:  e.winningHash,
          losingHash:   e.losingHash,
          method:       e.method,
          ts:           e.ts,
        });
        break;

      case EVENT_TYPES.ESA_HEIGHT_ADVANCED:
        esaHeight       = e.nextHeight ?? esaHeight;
        esaFinalizedCount = e.finalizedCount ?? esaFinalizedCount;
        break;
    }
  }
  ...
  return {
    ...
    forkResolutions, esaHeight, esaFinalizedCount,
